...
/*
 * 회원 가입 승낙 처리 (REQ_ACCOUNTS는 대기열 역할을 한다)
 */
UPDATE REQ_ACCOUNTS
   SET ISALLOWED = 'Y'
 WHERE ISALLOWED = 'P'
...
/*
 * 회원이 로그인 요청을 할 때 닉네임과 암호 확인하기 위한 조회
 */
SELECT ID
     , NICKNAME
     , PASSWORD
...
INSERT INTO SEQ_ACCOUNTS_ACCESS_LOGS VALUES (SEQ_ACCOUNT_ACCESS_LOGS.NEXTVAL
                                            , (SELECT ID FROM ACCOUNTS WHERE NICKNAME = '최가영' AND PASSWORD = 'samplepassword')
                                            , 'LOGIN', SYSDATE);
 ...
 *  로그아웃 할 때 LOG 테이블 기록
 */
INSERT INTO ACCOUNT_ACCESS_LOGS VALUES (SEQ_ACCOUNTS_ACCESS_LOGS.NEXTVAL, 9, 'LOGOUT', SYSDATE);
SELECT * FROM ACCOUNT_ACCESS_LOGS;

/*
 *  로그인을 한 회원의 정보를 조회할 때 몇 분 또는 몇 시간, 일, 개월 년 만에
 *  접속을 했는지 포함하여 조회
 */
SELECT FLOOR(SYSDATE - TO_DATE('20211029')) AS N일전에접속
     , FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('20211029'))) AS N개월전에접속
     , FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('20211029')) / 12) AS N년전에접속
     , TO_NUMBER(SYSDATE- TO_DATE('20221201')) * 24 * 60
  FROM DUAL;

/*
 *  회원이 암호 변경을 요청했을때 암호를 변경하기 위한 수정 구문
 */
